CREATE FUNCTION cmp.DemographicDim()
RETURNS TABLE AS
...
(
	SELECT CAST('$(ExpectedGraduationStartYear)' AS SMALLINT)
	UNION ALL
	SELECT CAST(ExpectedGraduationYear + 1 AS SMALLINT)
	FROM ExpectedGraduationSchoolYear
	WHERE ExpectedGraduationYear < CAST('$(ExpectedGraduationEndYear)' AS SMALLINT)
)
